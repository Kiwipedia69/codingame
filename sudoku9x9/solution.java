...
class Solution {
    ...
    static final int MaskALL = (1 << (N+1)) - 2;//0x03FFFFFE; // bits 1..N*N à 1 (bit 0 inutilisé)

    static int[][] g = new int[N][N];
    static int[] row = new int[N], col = new int[N], box = new int[N]; // bits 1..N*N
            ...
            for (int r = 0; r < N; r++) {
                ...
                if (unique != 0) {
                    for (int i = k; i < empties; i++) {
                        ...
                        int hit = m & unique;
                        if (hit != 0) {
                            // doit être 1 seul bit; sinon, input incohérent
    ...
    // ---- Solveur : MRV + tie-break degré (via voisins) + LCV (via voisins, cache used2/m2) + propagation ----
    ...
    public static void main(String[] args) throws Exception {
        ...
        //System.err.print("0x" + Integer.toHexString(MaskALL).toUpperCase());
        // Lecture + initialisation des masques + liste des vides
        for (int i = 0; i < N; i++) {
            String s = br.readLine();
            if (s == null || s.length() < N)
                throw new IllegalArgumentException("Ligne "+i+" invalide (longueur < N*N)");
            for (int j = 0; j < N; j++) {
                int d = s.charAt(j) - '0';;
                g[i][j] = d;
                if (d == 0) {
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) out.append(g[i][j]);
            out.append('\n');
        }
